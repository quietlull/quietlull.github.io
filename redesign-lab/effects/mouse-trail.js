/* WORKBENCH COPY, NOT THE SHIPPING FILE.
   The shipping version of this module is:
       _javascript/modules/components/mouse-trail.js
...
/* LAB COPY - taken verbatim from _javascript/modules/components/mouse-trail.js on 2026-08-22.
   WHY A COPY: D22 makes `_javascript/` off limits, and the handoff's rule is "if a lab page needs
   something from the live site, COPY IT INTO THE LAB; never edit the original."

   ROD, 2026-08-22: "the sparkler isnt being show across most of the new landing" (P66).
   ROOT CAUSE, and it is not a bug in this file: the sparkler lives in `commons.min.js`, and NO
   lab page loads that bundle. `final-landing.html` loads exactly two scripts - `hana-bloom.js` and
   `three-background-scene.min.js`. The sparkler was never on the page to fail. Measured: zero
   elements matching [class*=spark] on the rendered landing.

   THE ONLY EDITS to this file are the three import specifiers below, which gained a `.js`
   extension. Rollup resolves extensionless paths; a browser loading this as a plain ES module
   does not. Nothing else is changed - diff against the original to confirm.

   KNOWN LIMITATION, stated rather than discovered later: the effect binds its on/off state to a
   `.sparkler-switch` toggle in the top bar, and D20 removed the toggles from the bar. There is
   currently no manual control anywhere (that is REQUESTS #41/#45 territory), so the lab init
   turns it on directly. See sparkler-init.js.
*/
/**
...
import { parseRGB, isWarmColor } from './color-utils.js';
