/* Fireworks: make auto bursts actually reach the top of the screen.
   ...
   **20% and 70% down the viewport**. The top fifth is excluded by construction, every time. The
   ...
   WHY THIS IS STILL A RUNTIME PATCH, 2026-08-25, now that D22 is lifted and `_javascript/` is
   editable again. The real fix is one line in `_javascript/firework-controller.js:198` - the
   `randomY` range - and it is still the right end state. It is NOT done here because that file is
   Rollup input: changing it means rebuilding `three-background-scene.min.js` (843 KB) and
   `three-background-minimal.min.js` (525 KB) on merge night, both of them gitignored build output,
   while a second agent is mid-way through the CSS port. A rebuild that goes wrong takes the whole
   site down and there is no review in the morning if that happens.

   So: this stays, unchanged, and it costs nothing - it is a method override on an instance that
   may not exist, and it gives up quietly when it does not.
   TO FINISH IT: change line 198 of the controller to the two constants below, delete this file,
   and drop it from `_includes/chrome-scripts.html`. That is the whole removal.
